<template>
  <BasePage>
    <q-table
      ...
      :rows="rows"
      :columns="columns"
      :row-key="getNamespaceUid"
      :filter="filter"
      class="my-sticky-header-table fit flex-grow col overflow-auto"
        ...
        <q-tr class="cursor-pointer my-row" :props="props">
          <q-td key="current" class="cursor-pointer" :props="props">
            <q-icon
              size="xs"
              :name="
                selectedNamespace == props.row.metadata?.name ? 'mdi-check-circle-outline' : ''
              "
              @click="null"
            />
          </q-td>
          ...
          <q-td key="age" :props="props">
            <AgeDisplay
              :timestamp="props.row.metadata.creationTimestamp"
              :display-value="props.colsMap['age'].format(props.row.metadata?.creationTimestamp)"
            />
          </q-td>
          ...
          >
            <q-btn
              class="delete-btn full-width"
              flat
              push
              @click="applyNamespace(props.row)"
...
export default defineComponent({
  ...
  methods: {
    ...
    applyNamespace(row: V1Namespace) {
      if (row.metadata?.name && row.metadata?.name != this.selectedNamespace) {
  ...
  computed: {
    ...mapState(useK8DataStore, [
      'getAllNamespaceItems',
      'status',
      'message',
      'selectedNamespace',
      'shouldRefreshNamespaces',
    ]),
    rows() {
      ...
      columns: [
        {
          name: 'current',
          label: 'Current',
          field: (r: V1Namespace) => r?.metadata?.name,
        ...
        },
        {
          name: 'age',
          label: 'Age',
          field: (r: V1Namespace) => r.metadata?.creationTimestamp,
          align: 'center' as const,
          format: (val: string) => {
